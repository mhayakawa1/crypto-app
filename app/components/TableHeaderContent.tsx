...
const TableHeaderContent = (props: { updateValue: any, mobileView: boolean }) => {
  ...
  return (
    <TableRow className="h-[50px] lg:2xl:h-[75px] hover:bg-transparent flex items-center justify-start gap-[8px] lg:2xl:gap-[12px] p-0 border-none">
      {headerInfo.map((element: any) => {
        const { name, sortValue, width } = element;
        return (
          <TableHead
            key={name}
            className={`${width} h-full flex items-center justify-center p-0 lg:2xl:text-2xl`}
          >
            {sortValue !== null ? (
              <SortButton
                name={name}
