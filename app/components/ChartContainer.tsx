...
const ChartContainer = (props: {
  ...
  return (
    <div
      className={`flex flex-col justify-between gap-[4vh] grow bg-white dark:bg-[--mirage] text-[--american-blue] dark:text-white px-[2vw] max-md:px-[4vw] py-[24px] max-sm:pt-[16px] max-sm:pb-[4px] lg:2xl:py-[48px] rounded-[16px] lg:2xl:rounded-[32px] ${className}`}
    >
      {typeof chartInfo === "string" ? (
        <h3 className="lg:2xl:text-2xl">{chartInfo}</h3>
      ) : compareData ? (
        <ul className="text-[--mirage] dark:text-[--light-gray]">
          <li className="text-[28px]/[24px] dark:text-white pt-[24px] pb-[16px] font-bold">
        ...
        <ul className="text-[--mirage] dark:text-[--light-gray] max-sm:flex justify-between">
          ...
          <li className="flex flex-col gap-[16px] max-sm:gap-[8px] dark:text-white pt-[24px] max-sm:pt-0">
            <span className="text-2xl max-sm:text-xl font-bold">
            {formatNumber(value, symbol)}</span>
            <span className="text-base max-sm:text-xs">{formattedDate}</span>
          </li>
        </ul>
      )}
